...
import { Tabs, TabList, TabPanels, Tab, TabPanel, Box, Image, Flex, Tooltip, useBreakpointValue } from '@chakra-ui/react'
...
<Tabs>
  ...
  <TabList  justifyContent={"center"} borderBottom={"none"} >
    ...
    {showTooltip1 && (
        <Tooltip label="Discover events based on the Popular Categories." aria-label="Tooltip 1" placement="top">
          <Tab _selected={{borderBottom:'none'}}>
        <Image  src="/tabicon-1.svg" alt='imgicon'/>
    </Tab>
        </Tooltip>
      )}
      {!showTooltip1 && <Tab _selected={{borderBottom:'none'}}>
        <Image  src="/tabicon-1.svg" alt='imgicon'/>
    </Tab>}
   
      {showTooltip2 && (
        <Tooltip label="Discover events based on the location you enter." aria-label="Tooltip 2" placement="top">
          <Tab _selected={{borderBottom:'none'}}>
    <Image    src="/tabicon-2.svg" alt='imgicon'/>
    </Tab>
        </Tooltip>
      )}
      {!showTooltip2 && <Tab _selected={{borderBottom:'none'}}>
    <Image    src="/tabicon-2.svg" alt='imgicon'/>
    </Tab>}
      ...
      {showTooltip3 && (
        <Tooltip label="Discover events by selecting dates on the calendar." aria-label="Tooltip 3" placement="top">
          <CalendarDropdown/>
        </Tooltip>
      )}
